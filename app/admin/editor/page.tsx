...
import { 
  ...
  Palette, Crosshair, Rocket, Zap, Sliders, Check, Monitor
} from "lucide-react";
...
function EditorContent() {
  ...
  const [aiMessages, setAiMessages] = useState<{role: string, text: string}[]>([
    { role: 'ai', text: '艦長！系統全武裝已掛載。AI 營運顧問隨時為您服務。' }
  ]);
  ...
  // 分頁管理功能 (全武裝回歸)
  const addPage = () => {
    if (pages.length >= 6) { alert("⚠️ 最多只能建立 6 個分頁！"); return; }
    saveHistory();
    setPages([...pages, { id: `p${Date.now()}`, name: `新分頁 ${pages.length + 1}`, imageUrl: null, areas: [], chatBarText: '✨' }]);
  ...
  const handleExport = async () => { setIsExporting(true); await new Promise(r => setTimeout(r, 1200)); setIsExporting(false); alert("🚀 專案已打包並發布至正式環境！"); };

  const askGemini = async () => {
    if (!aiInput.trim()) return;
    setAiMessages(prev => [...prev, { role: 'user', text: aiInput }]); setAiInput(""); setIsAiLoading(true);
    setTimeout(() => {
      setAiMessages(prev => [...prev, { role: 'ai', text: "我已收到您的指令。這項功能正在與您的專屬後端對接中，敬請期待。" }]);
  ...
  return (
    <main className="h-screen flex flex-col overflow-hidden font-sans select-none relative transition-colors duration-300" style={{ backgroundColor: theme.bgMain, color: theme.textMain, fontSize: `${theme.fontSize}px` }} onMouseUp={() => setMode('idle')}>
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-2 mt-1">
               <div className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ backgroundColor: theme.primary }}></div>
               <span className="uppercase tracking-[0.2em] font-mono font-bold" style={{ color: theme.textDim, fontSize: `${theme.fontSize * 0.75}px` }}>V17.0 Full Armor Edition</span>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <button onClick={undo} disabled={history.length === 0} className="px-5 py-2.5 rounded-xl border font-bold disabled:opacity-20 hover:opacity-80 flex items-center gap-2 transition-all cursor-pointer" style={{ backgroundColor: theme.bgInput, borderColor: currentBorder, color: theme.textDim, borderRadius: `${theme.radius}px`, fontSize: `${theme.fontSize}px` }}><RotateCcw size={16}/> 撤銷</button>
          <button onClick={() => setShowPreview(true)} className="px-6 py-2.5 rounded-xl font-bold border hover:opacity-80 flex items-center gap-2 transition-all cursor-pointer" style={{ backgroundColor: theme.bgInput, borderColor: theme.primary, color: theme.primary, borderRadius: `${theme.radius}px`, fontSize: `${theme.fontSize}px` }}><Eye size={16}/> 模擬器</button>
          <button onClick={async () => { setSaving(true); await updateDoc(doc(db, "users", uid!), { megaMenuConfig: { pages, theme } }); setSaving(false); alert("✅ 儲存成功！"); }} className="px-8 py-2.5 rounded-xl font-black text-white shadow-lg active:scale-95 flex items-center gap-2 transition-all cursor-pointer" style={{ backgroundColor: theme.primary, borderRadius: `${theme.radius}px`, fontSize: `${theme.fontSize}px` }}>{saving ? <RefreshCw className="animate-spin" size={16}/> : <Save size={16}/>} 儲存</button>
        </div>
      </header>
      ...
      <div className="flex-1 flex overflow-hidden relative z-10">
        ...
        <aside className="border-r flex flex-col shrink-0 relative z-30 shadow-xl transition-colors duration-300" style={{ width: "320px", backgroundColor: theme.bgPanel, borderColor: currentBorder }}>
          <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
            <button onClick={() => setShowThemeModal(true)} className="w-full p-4 flex items-center justify-center gap-2 border hover:brightness-125 shadow-sm font-black transition-all cursor-pointer" style={{ backgroundColor: `${theme.primary}15`, borderColor: `${theme.primary}50`, color: theme.primary, borderRadius: `${theme.radius}px`, fontSize: `${theme.fontSize}px` }}><Palette size={16} /> 視覺主題設定</button>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                {pages.length < 6 && <button onClick={addPage} className="p-1 hover:bg-white/10 rounded cursor-pointer transition-colors" style={{ color: theme.textDim }}><span className="text-lg leading-none">+</span></button>}
              </div>
              <div className="space-y-2">
                {pages.map((page, index) => (
                  <div key={page.id} onClick={() => { setActivePageIndex(index); setSelectedId(null); }} className="group flex items-center justify-between p-3 transition-all border cursor-pointer" style={{ backgroundColor: activePageIndex === index ? theme.textMain : theme.bgInput, color: activePageIndex === index ? theme.bgMain : theme.textDim, borderColor: activePageIndex === index ? theme.textMain : currentBorder, borderRadius: `${theme.radius > 8 ? 8 : theme.radius}px`, fontSize: `${theme.fontSize * 0.9}px` }}>
                    ...
                    {pages.length > 1 && <button onClick={(e) => deletePage(e, index)} className="opacity-0 group-hover:opacity-100 hover:text-red-500 transition-opacity cursor-pointer"><X size={14}/></button>}
                  </div>
                ))}
              </div>
            </div>
            ...
            <section className="p-5 border transition-all" style={{ backgroundColor: theme.bgInput, borderRadius: `${theme.radius}px`, borderColor: currentBorder }}>
              <h3 className="font-black uppercase tracking-widest mb-3 flex items-center gap-2" style={{ color: theme.textDim, fontSize: `${theme.fontSize * 0.8}px` }}><Type size={14} style={{ color: theme.primary }}/> 底部標籤設定</h3>
          ...
          <div className="p-6 border-t shrink-0" style={{ borderColor: currentBorder, backgroundColor: theme.bgHeader }}>
            <button onClick={handleExport} className="w-full py-4 flex items-center justify-center gap-2 font-black uppercase tracking-widest transition-all shadow-xl hover:brightness-110 active:scale-95 cursor-pointer" style={{ backgroundColor: theme.primary, color: "#fff", borderRadius: `${theme.radius}px`, fontSize: `${theme.fontSize}px` }}>
              {isExporting ? <RefreshCw className="animate-spin" size={16}/> : <Rocket size={16}/>} 發布至生產環境
            </button>
          </div>
        </aside>
            ...
            {!activePage.imageUrl ? (
              ...
              <div ref={containerRef} className="relative shadow-[0_30px_80px_rgba(0,0,0,0.6)] inline-block cursor-crosshair ring-1" style={{ ringColor: currentBorder, borderRadius: `${theme.radius > 16 ? 16 : theme.radius}px`, backgroundColor: theme.bgInput }} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove}>
                ...
                {activePage.areas.map((area:any) => (
                  <div key={area.id} data-id={area.id} className={`hot-area absolute border-2 flex items-center justify-center ${selectedId === area.id ? 'cursor-move z-30 shadow-lg' : 'cursor-pointer transition-colors z-20 hover:bg-white/10'}`} style={{ left:`${area.x}%`, top:`${area.y}%`, width:`${area.w}%`, height:`${area.h}%`, borderColor: selectedId === area.id ? theme.primary : "rgba(255,255,255,0.4)", backgroundColor: selectedId === area.id ? `${theme.primary}30` : "transparent", borderRadius: `${theme.radius > 8 ? 8 : theme.radius}px` }}>
                    {selectedId === area.id && (
                      <>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        {[ { id: 'switch', label: '跳轉分頁', icon: ArrowRightLeft }, { id: 'card', label: '數位名片', icon: Smartphone }, { id: 'uri', label: '外部網址', icon: LinkIcon }, { id: 'message', label: '自動回覆', icon: FileText } ].map(btn => (
                          <button key={btn.id} onClick={() => updateArea(activeArea.id, 'type', btn.id)} className="flex flex-col items-center justify-center gap-2 p-3 border font-bold transition-all hover:opacity-80 cursor-pointer shadow-sm" style={{ backgroundColor: activeArea.type === btn.id ? theme.primary : "transparent", borderColor: activeArea.type === btn.id ? theme.primary : currentBorder, color: activeArea.type === btn.id ? "#fff" : theme.textDim, borderRadius: `${theme.radius > 12 ? 12 : theme.radius}px`, fontSize: `${theme.fontSize * 0.9}px` }}>
      ...
      {/* 🤖 AI 對話顧問 (右下角懸浮) */}
      <div className={`fixed bottom-8 right-8 z-[50] transition-all duration-300 ease-out flex flex-col justify-end items-end ${showAI ? 'w-[360px] h-[520px]' : 'w-16 h-16'}`}>
        {!showAI ? (
          <button onClick={() => setShowAI(true)} className="w-16 h-16 flex items-center justify-center shadow-[0_10px_30px_rgba(0,0,0,0.5)] hover:scale-105 transition-transform text-white border border-white/20 cursor-pointer" style={{ backgroundColor: theme.primary, borderRadius: `${theme.radius > 24 ? 24 : theme.radius}px` }}>
            <Sparkles size={24} />
          </button>
        ) : (
          <div className="w-full h-full border flex flex-col overflow-hidden shadow-[0_30px_100px_rgba(0,0,0,0.8)]" style={{ backgroundColor: theme.bgPanel, borderColor: currentBorder, borderRadius: `${theme.radius > 24 ? 24 : theme.radius}px` }}>
            <div className="px-5 py-4 border-b flex justify-between items-center shrink-0" style={{ backgroundColor: theme.bgHeader, borderColor: currentBorder }}>
              <div className="flex items-center gap-2 font-black tracking-widest uppercase" style={{ color: theme.textMain, fontSize: `${theme.fontSize * 0.9}px` }}><Sparkles size={16} style={{ color: theme.primary }}/> AI 營運顧問</div>
              <button onClick={() => setShowAI(false)} className="hover:opacity-50 transition-opacity cursor-pointer" style={{ color: theme.textDim }}><X size={18}/></button>
            </div>
            <div className="flex-1 overflow-y-auto p-5 space-y-5 bg-black/10 custom-scrollbar">
              {aiMessages.map((m, i) => (
                <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2`}>
                  <div className={`max-w-[85%] px-4 py-3 leading-relaxed shadow-md ${m.role === 'user' ? 'rounded-tr-none' : 'rounded-tl-none border'}`} style={{ backgroundColor: m.role === 'user' ? theme.primary : theme.bgInput, color: m.role === 'user' ? "#fff" : theme.textMain, borderColor: m.role === 'user' ? "transparent" : currentBorder, borderRadius: `${theme.radius > 16 ? 16 : theme.radius}px`, fontSize: `${theme.fontSize}px` }}>{m.text}</div>
                </div>
              ))}
            ...
            <div className="p-4 border-t flex gap-2 shrink-0" style={{ backgroundColor: theme.bgHeader, borderColor: currentBorder }}>
              ...
              <button onClick={askGemini} className="px-4 text-white shadow-sm flex items-center justify-center hover:brightness-110 active:scale-95 cursor-pointer" style={{ backgroundColor: theme.primary, borderRadius: `${theme.radius > 12 ? 12 : theme.radius}px` }}><Send size={16}/></button>
            </div>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4 pointer-events-auto" style={{ zIndex: 99999 }}>
          ...
          <div className="relative w-[860px] h-[600px] bg-[#0a0a0a] border border-white/10 rounded-2xl shadow-[0_40px_100px_rgba(0,0,0,0.9)] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="h-[60px] px-6 border-b border-white/10 flex justify-between items-center shrink-0 bg-[#111111]">
              <div className="flex items-center gap-3"><div className="p-1.5 rounded-lg bg-white/5"><Palette size={18} className="text-white"/></div><h2 className="text-base font-black text-white tracking-widest uppercase">主題風格工作站</h2></div>
              <button onClick={() => setShowThemeModal(false)} className="p-1.5 hover:bg-white/10 rounded-full transition-colors text-gray-400 hover:text-white cursor-pointer"><X size={18}/></button>
            </div>
            <div className="flex-1 flex overflow-hidden">
              <div className="w-[320px] shrink-0 border-r border-white/10 flex flex-col bg-[#111111]">
                <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
                  <section>
                    <div className="flex items-center gap-2 mb-3 text-white"><Zap size={14}/><span className="font-black uppercase tracking-widest text-[10px]">快速風格套用</span></div>
                    <div className="grid grid-cols-2 gap-2">{Object.entries(THEME_PRESETS).map(([name, p]) => (
                      <button key={name} onClick={() => setTheme(p)} className="flex items-center justify-between p-3 rounded-lg border border-white/10 bg-white/5 hover:bg-white/10 text-[10px] font-bold text-gray-300 group"><div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full shadow-sm" style={{ backgroundColor: p.primary }}></div><span className="group-hover:text-white transition-colors">{name}</span></div>{theme.primary === p.primary && <Check size={14} className="text-white"/>}</button>
                    ))}</div>
                  </section>
                  <section className="space-y-3">
                    <div className="flex items-center gap-2 mb-3 text-white"><Sliders size={14}/><span className="font-black uppercase tracking-widest text-[10px]">核心色彩定義</span></div>
                    {[ { l: "品牌色", k: "primary" }, { l: "主要背景", k: "bgMain" }, { l: "標題底色", k: "bgHeader" }, { l: "面板底色", k: "bgPanel" }, { l: "主要文字", k: "textMain" } ].map(item => (
                      <div key={item.k} className="flex justify-between items-center p-2.5 rounded-lg border border-white/10 bg-black/40 hover:bg-black/60 transition-colors"><span className="font-bold text-gray-300 text-xs px-1">{item.l}</span><input type="color" value={(theme as any)[item.k]} onChange={(e) => setTheme({...theme, [item.k]: e.target.value})} className="w-6 h-6 rounded border-0 bg-transparent cursor-pointer"/></div>
                    ))}
                  </section>
                  <section className="space-y-4 pb-2">
                    <div className="flex items-center gap-2 mb-3 text-white"><Sliders size={14}/><span className="font-black uppercase tracking-widest text-[10px]">幾何與細節</span></div>
                    {[ { l: "圓角幅度", k: "radius", m: 0, x: 32, s: 2, u: "px" }, { l: "邊框透感", k: "borderOpacity", m: 0, x: 0.5, s: 0.01, u: "" }, { l: "介面字體", k: "fontSize", m: 10, x: 18, s: 1, u: "px" } ].map(s => (
                      <div key={s.k} className="space-y-3 p-4 rounded-lg border border-white/10 bg-black/40"><div className="flex justify-between items-center font-bold"><span className="text-gray-300 text-xs px-1">{s.l}</span><span className="font-mono bg-white/10 px-1.5 py-0.5 rounded text-white text-[10px]">{(theme as any)[s.k]}{s.u}</span></div><input type="range" min={s.m} max={s.x} step={s.s} value={(theme as any)[s.k]} onChange={(e) => setTheme({...theme, [s.k]: parseFloat(e.target.value)})} className="w-full h-1 bg-white/20 rounded-lg appearance-none cursor-pointer accent-white"/></div>
                    ))}
                  </section>
                </div>
              </div>
              <div className="flex-1 bg-[#050505] relative flex flex-col items-center justify-center p-8 bg-[radial-gradient(#ffffff08_1px,transparent_1px)] [background-size:20px_20px]">
                 <div className="absolute top-6 flex items-center gap-1.5 px-4 py-1.5 rounded-full border border-white/10 shadow-sm backdrop-blur-md bg-white/5"><Monitor size={12} className="text-gray-400"/><span className="font-black uppercase tracking-[0.2em] text-gray-400 text-[9px]">Live Preview</span></div>
                 <div className="w-[360px] shrink-0 border shadow-[0_20px_50px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden transition-all duration-300" style={{ backgroundColor: theme.bgPanel, borderColor: currentBorder, borderRadius: `${theme.radius * 1.5}px` }}>
                    <div className="h-12 border-b flex items-center px-5" style={{ backgroundColor: theme.bgHeader, borderColor: currentBorder }}><div className="w-3 h-3 rounded-full shadow-sm" style={{ backgroundColor: theme.primary }}></div></div>
                    <div className="p-6 space-y-5 text-center flex-1 flex flex-col justify-center">
                       <button className="w-full py-4 font-black shadow-lg cursor-default transition-all text-xs" style={{ backgroundColor: theme.primary, color: "#fff", borderRadius: `${theme.radius}px` }}>DEMO BUTTON</button>
                    </div>
                 </div>
              </div>
            </div>
            <div className="h-[60px] px-8 border-t border-white/10 flex justify-end items-center shrink-0 bg-[#111111]">
               <button onClick={() => setShowThemeModal(false)} className="px-8 py-2 font-black text-black bg-white rounded-lg shadow-lg hover:brightness-110 active:scale-95 transition-all text-xs cursor-pointer">套用變更</button>
            </div>
          </div>
        </div>
      )}
